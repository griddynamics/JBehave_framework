...
android_prerequisites () {
  ...
  : '(3)' &&   cd $APPIUM_HOME;
               ...
               screen -dmS Appium nohup node ./lib/server/main.js --device-ready-timeout 120 --log-no-colors -g $BUILD_DIR/../server.log;
}
...
android_connect () {
        ...
        count=0;
        until [ $($ADB_PATH/adb wait-for-device && $ADB_PATH/adb shell dumpsys window | grep mCurrentFocus | wc -l || echo -1) -eq 1 ]; do
          printf "*";
          sleep 15
...
: '(1)' &&     kill $(jps -m | grep selendroid-standalone | awk -F " " '{print $1}') 2>/dev/null;
               execute --in_background "java -jar $APPIUM_HOME/submodules/selendroid/selendroid-standalone/target/selendroid-standalone-0.13.0-with-dependencies.jar 1>$BUILD_DIR/../server.log 2>>$BUILD_DIR/../server.log;" 1>/dev/null 2>/dev/null;
