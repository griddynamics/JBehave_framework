...
echo "[INFO] executing ios_prerequisites function...";
  ...
  : '(2)' &&   mkdir -p $APPIUM_HOME/build/SafariLauncher >/dev/null 2>/dev/null;
               cp -f $BUILD_DIR/ios/SafariLauncher.zip $APPIUM_HOME/build/SafariLauncher/ >/dev/null 2>/dev/null;
               cd $APPIUM_HOME;
               test -z `cat $APPIUM_HOME/.appiumconfig* | grep -o ios | head -1` && ./reset.sh --android --selendroid --ios;
